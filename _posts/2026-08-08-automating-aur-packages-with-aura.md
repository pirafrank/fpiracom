---
title: "Automating AUR Packages with AURA and GitHub Actions"
subtitle: "Generating PKGBUILD and .SRCINFO files from GitHub Releases and publishing to the Arch User Repository"
...
![Automating AUR packages with AURA and GitHub Actions]({{ site.baseurl }}/static/postimages/3019/aura-aur-package-automation.svg)

In [part 1]({% post_url 2026-08-01-automating-homebrew-tap-from-github-releases %}), I described the basic philosophy behind this series: a GitHub Release stays the source of truth, and packaging metadata is generated from it.
...
- [poof-bin](https://aur.archlinux.org/packages/poof-bin)
- [vault-conductor-bin](https://aur.archlinux.org/packages/vault-conductor-bin)
...
I wanted a name that was short, memorable, and evocative of Arch Linux. AURA is a good fit for multiple reasons:

- it is short and easy to type (I am lazy at the keyboard)
- it has *AUR* in it which is evocative of Arch Linux User Repository
- it is a real word in English, it means *a distinctive atmosphere or quality that seems to surround and be generated by a person, place, or thing*
- and... it's also a little joke to my wife, with which I watched *What's Wrong with Secretary Kim*, a K-Drama where a narcissistic boss repeats *aura* as a pun to his secretary Kim Mi-so (the Korean word for "aura" is "mi-so" (미소)).
...
| Submodule path               | AUR repository URL                             |
|------------------------------|------------------------------------------------|
...
## PKGBUILD is not a Formula

For Homebrew, my output file is Formula.rb. For Arch, the important output is usually `PKGBUILD`, plus the generated `.SRCINFO`.
...
AUR pushes happen over SSH to the AUR-hosted repository. In my CI workflow, I also sign commits, so I turned to [vault-conductor](https://github.com/pirafrank/vault-conductor) to manage both signing and SSH authentication. With it the workflow can:

- sign the commit with the SSH signing key
- authenticate the push to `aur.archlinux.org`

That is a much more specific workflow than "render a file and commit it" over GitHub GraphQL APIs.
...
I made AURA work for me because by respecting Arch packaging instead of trying to hide it: it keeps AUR repositories as submodules, uses a small config-and-template model, generates `.SRCINFO` automatically, and lets GitHub Actions perform the repetitive part while still preserving the publication model the AUR expects.
...
The final article in the series will go one step further: it will cover building and publishing tools for Debian-like, RPM-based, and Alpine systems, where a package file is only part of the problem. You also need repository indexes, signatures, storage, and a serving path users can actually add to their systems.
