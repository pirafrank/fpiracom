...
I ran into this when I wanted to distribute [poof](https://github.com/pirafrank/poof) and [vault-conductor](https://github.com/pirafrank/vault-conductor) through the package managers users already have. I mostly develop CLI tools, with Rust being my language of choice. It has been like that for quite some time. That means I can cross-compile binaries for Linux and macOS from a [single CI job](https://github.com/pirafrank/poof/blob/9137bcdc8b3ee58042d940e1ae5da49401a46d30/.github/workflows/ci_cross.yml), and I can produce a GitHub Release with all the assets I need.

It was already two tools with a similar release flow, and I knew I would have more to add in the future (my projects shelf already has some drafts). So, I did not want to manually repeat the same packaging work for each new release of the tools.

> "When you try to solve a problem, you should not create a new one."

## What I did

After looking at the distributions and package managers I wanted to support, I decided that, besides version managers like [asdf](https://asdf-vm.com/), the tools should be first-class citizens on macOS and on Linux. The goal was to meet users where they already are, with tools they know and use.

I split the problem into three repositories:
...
The main idea behind all three repositories is the same. A GitHub Release remains the source of truth, and the package manager metadata is generated from it and automatically committed via a GitHub Actions workflow. That keeps the packaging side small, repeatable, and easy to debug when something goes wrong.

As a side note, Homebrew is only available on [a limited set](https://docs.brew.sh/Support-Tiers) of platforms (namely ARM64/AArch64 and Intel x86_64 with SSSE3 support), and I wanted to make my tools available on as many as possible, that's why I have built [poof](https://poof.fpira.com). But that's a story for another post.
...
The user-facing flow stays simple as usual:
...
This is for example to install `poof`. Exactly the outcome I want. The interesting work should happen before the user ever runs the install command.
...
The metadata configuration used for `poof` follows. It contains naming conventions of assets for x86_64 and arm64 variants which cover the CPU architectures best supported by Homebrew on Linux and macOS.
...
- `{VERSION}` becomes the latest release version, such as `0.6.2`
...
That is one reason I care so much about cross-platform release naming. I wrote about that part [a couple of posts ago]({% post_url 2025-01-20-cross-compilation-in-rust %}).
...
The first is exact asset matching. The script does not do fuzzy guessing. If it expects `poof-0.6.2-x86_64-apple-darwin.tar.gz` and that asset is missing, it fails loudly. That is much better than silently generating a broken formula.
...
The commit step is also intentionally conservative. The workflow checks for diffs first. If nothing changed, there is nothing to commit. If something changed, the workflow creates a signed commit with [github-commit-sign](https://github.com/marketplace/actions/github-commit-sign){:target="_blank"}{:rel="noopener noreferrer"} action.

That signing step matters because packaging metadata is part of the trust story. Users are not only trusting the binary they install, they are also trusting the path that points them to it. I wrote [a separate post]({% post_url 2021-11-22-git-sign-commits-and-tags %}) about commit and tag signing in Git.
...
The Homebrew tap works well because it stays small. It does not compile the software. It does not build repository indexes. It does not try to push to AUR. It only consumes release metadata and emits Homebrew metadata.
...
In the next article, I will look at [AURA](https://github.com/pirafrank/aura). Arch packaging solves the same problem, but its conventions are different enough that I keep it completely separate.
